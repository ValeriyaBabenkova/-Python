...
class TaskModelSQL:
    ...
    def add_task(self, text, user_id, task_status):
        connection = sqlite3.connect(self.db_name)
        cursor = connection.cursor()
        cursor.execute('insert into task (user_id, name, status) values (?, ?, ?)', (user_id, text, task_status))
        connection.commit()
        connection.close()

    def change_status(self, status, task_id):
        connection = sqlite3.connect(self.db_name)
        cursor = connection.cursor()
        cursor.execute('update task set status = ? where task.id = ?', (status, task_id))
        connection.commit()
        connection.close()
...
def start(message):
    ...
    button_change = types.KeyboardButton('Изменить статус задачи')
    markup.add(button_add, button_tasks, button_del, button_change)
...
def delete_task(message):
    ...
    for task in list_tasks:
        task_string += f"{task['name']} (id задачи - {task['id']}). Статус: {task['status']}.\n"
...
def get_task(message):
    ...
    if user_state == ADD_STATE:
        task_status = 'сделать'
        db.add_task(message.text, user['id'], task_status)
        user_state = ''
        ...
        for task in list_tasks:
            if task_id == task['id']:
                if task['status'] != 'сделано':
                    task['status'] = 'сделано'
                    db.change_status(task['status'], task_id)
